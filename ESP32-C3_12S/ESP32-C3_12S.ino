...
#define WIFI_FAILED_ROUNDS_BEFORE_REBOOT 3
...
void connectWiFiUntilSuccess() {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    failedRounds++;
    Serial.printf("正在连接 WiFi: %s (第 %d 轮)\n", wifiSsid.c_str(), failedRounds);
    ...
    Serial.printf("\n本轮连接失败，status=%d，%d 秒后重试...\n", WiFi.status(), WIFI_RETRY_GAP_MS / 1000);
    if (failedRounds >= WIFI_FAILED_ROUNDS_BEFORE_REBOOT) {
      Serial.println("连续多轮连接失败，重启设备后再试...");
      delay(1000);
      ESP.restart();
    }
...
void setup() {
  ...
  Serial.println("ESP32-C3 噪音传感器已启动，网页服务与上传任务已分离");
}
